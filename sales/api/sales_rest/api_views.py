...
def api_list_customer(request):
    if request.method == "GET":
        ...
        return JsonResponse({"customer": customer}, encoder=CustomerEncoder, safe=False)
    else:
        try:
...
def api_show_customer(request, id):
    if request.method == "GET":
        try:
            ...
            response = JsonResponse({"message": "Unable to find Customer"})
...
def api_list_sales_person(request):
    if request.method == "GET":
        ...
        return JsonResponse(
            {"sales_person": sales_person}, encoder=SalesPersonEncoder, safe=False
        )
    else:
        try:
            content = json.loads(request.body)
            sales_person = SalesPerson.objects.create(**content)
            return JsonResponse(sales_person, SalesPersonEncoder, safe=False)
        except:
            response = JsonResponse(
                {"message": "Person does not exist"},
            )
...
def api_show_sales_person(request, id):
    if request.method == "GET":
        try:
            ...
            response = JsonResponse({"message": "Unable to find Sales Person"})
    elif request.method == "DELETE":
        try:
            sales_person = SalesPerson.objects.get(id=id)
            sales_person.delete()
...
def api_list_sales(request):
    if request.method == "GET":
        ...
        return JsonResponse({"sales": sales}, encoder=SaleRecordEncoder, safe=False)
    else:
        try:
            content = json.loads(request.body)
            ...
            print(sale_record)
            return JsonResponse(sale_record, encoder=SaleRecordEncoder, safe=False)
        except:
            response = JsonResponse({"message": "Unable to make Sales record"})
            response.status_code = 400
            return response
...
def api_show_sale(request, id):
    if request.method == "GET":
        try:
            ...
            return JsonResponse(sales, SaleRecordEncoder, safe=False)
        except:
            response = JsonResponse({"message": "Unable to find Sale Record"})
    elif request.method == "DELETE":
        try:
            sales = SaleRecord.objects.get(id=id)
            sales.delete()
            return JsonResponse(sales, encoder=SaleRecordEncoder, safe=False)
        except:
...
def api_show_sales_history(request, id):
    if request.method == "GET":
        ...
        return JsonResponse(
            {"sales_history": sales_history}, SaleRecordEncoder, safe=False
        )
    else:
        content = json.loads(request.body)
        automobile = AutoVO.objects.get(id=id)
        if automobile.is_sold == False:
            ...
            return JsonResponse(
                {"sale_record": sale_record}, SaleRecordEncoder, safe=False
            )
        else:
            response = JsonResponse("Vehicle is sold already")
            response.status_code = 400
            return response
...
def api_get_auto_vo_list(request):
    if request.method == "GET":
        ...
        return JsonResponse(automobiles, AutoVOEncoder, safe=False)
...
def api_get_sales_record_person(request, id):
    if request.method == "GET":
        ...
        return JsonResponse(
            {"sales_person": sales_person}, SaleRecordEncoder, safe=False
        )
